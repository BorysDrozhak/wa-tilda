...
from .gspread_api import add_history, get_previous_date_total, get_records, update_total_records, create_creds_json, CREDENTIALS_DICT
from .weather_cli import get_whether_forecast, kyiv_timezone
from const import (
    DELIVERY_NET_RATE,
...
def parse_total_kassa(text, env):
    ...
    z_zvit = 0
    data = []
    zvit_date = datetime.datetime.now(tz=kyiv_timezone).strftime('%m/%d/%Y')
    week_difference = 0
    ...
    if data:
        add_history(data, zvit_date.strftime('%m/%d/%Y'), creds=gc_creds)
    previous_week_total = get_previous_date_total(zvit_date - datetime.timedelta(days=7), creds=gc_creds)
    ...
    new_records = ""
    delivery_records_dict, resto_records_dict = get_records(creds=gc_creds)
    ...
    if top_delivery_record != 0 and int(total_delivery) > top_delivery_record:
        ...
        update_total_records(delivery_record_data, 'Доставка', creds=gc_creds)
    ...
    if top_resto_record != 0 and int(total_resto) > top_resto_record:
        ...
        update_total_records(resto_record_data, 'Зал', creds=gc_creds)
