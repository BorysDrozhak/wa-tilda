...
async def onboarding_message(user_info, context):
    ...
    await context.bot.send_message(
        chat_id=str(Channels.wa_resto_hall_channel.value),
        text=f'{user_info.get("username")}, це основна група команди залу 🙌🏻\n\n'
...
async def add_members(user_info, update, context):
    ...
    try:
        ...
        print(e)
        await update.message.reply_text('Упс!\nЩось пішло не так(\nСпробуйте ще раз')
    else:
        add_user_data(user_data)
        await update.message.reply_text('Інформація про працівника успішно збережена!')
